...
const DashboardIndex = () => {
  const [reservationdetailsidebar, setReservationDetailSidebar] =
    useState(false);
  const [reservationid, setReservationId] = useState("");
  const dispatch = useDispatch();
  const { reservations, getsingleReservationisLoading, page } = useSelector(
  ...
  return (
    <div className="w-full flex flex-col gap-8 lg:gap-16  md:gap-12">
      ...
      <div className="w-full grid items-start gap-12 lg:gap-12 lg:grid-cols-1">
        ...
        <BookingDataTable
          title={"Reservation listings!"}
          data={data}
          description={"Here's a list of your reservation for this month."}
        />
      </div>
    </div>
  );
